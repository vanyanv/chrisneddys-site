...
- Admin: confirmations in Settings no longer wait on a full page reload. Removing an owner or sending an invite told you it had worked only after the whole Settings page had re-read itself, so on a slow database the confirmation could take many seconds to appear, or appear not to come at all. The work itself was always done; only the message was late ([#38](https://github.com/vanyanv/chrisneddys-site/issues/38))
