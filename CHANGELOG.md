...
- Admin: Orders, the order page, Settings and the packing slip move onto the same system as Today and Products, so the whole admin looks like one thing again. Refunding an order now asks first, in a panel that says plainly there is no undo, and sends the customer their confirmation ([#36](https://github.com/vanyanv/chrisneddys-site/issues/36))
...
- Admin: Products is now The Rack's catalogue — a card grid with a detail panel, instead of the old expanding table. Adding a product opens straight onto a blank draft with no name yet, rather than asking for one up front; that empty state is honest everywhere it shows, and a draft can't go live until it actually has a name, a price, a run size and a photo. First-run and loading states are real screens now instead of a blank page. Price, run size, Live/Draft/Archive, reordering, photos and duplicating all still work the same as before ([#36](https://github.com/vanyanv/chrisneddys-site/issues/36))
