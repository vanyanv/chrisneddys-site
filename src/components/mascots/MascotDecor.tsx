...
export type MascotDecorKind = "bullseye" | "diamond" | "drip" | "court";
...
const COLOR_A_VAR: Record<MascotDecorKind, "--op-a" | "--drip-color" | "--court-color"> = {
  bullseye: "--op-a",
  diamond: "--op-a",
  drip: "--drip-color",
  court: "--court-color",
 ...
 * diamonds, paint drips and the half-court sketch — drawn by referencing
 * `MascotDefs`' shared symbols the same way `Monster` does. `colorB` only
 * applies to the two-tone symbols (bullseye, diamond); everything else takes
 * a single color through `colorA`.
 */
