 ...
 * painted inside the same `<svg>`, after the `<use>`, it covers the symbol's
 * own iris with the eye white and redraws the iris and pupil in a group the
 * caller can move (the hero's pointer-tracking `Watcher`, the last-call
 * banner's darting eye). The iris is clipped to the eye white, so however far
 * it moves it never crosses the ring. The shapes are the ones `MascotDefs`
